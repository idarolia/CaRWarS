...
int objloader::load(const char* filename){
        ...
        for(int i=0;i<coord.size();i++){                                //and then go through all line and decide what kind of line it is
                ...
                }else if((*coord[i])[0]=='v' && (*coord[i])[1]=='t')            //back to the obj file, texture coorinate
                {
                        ...
                        texturecoordinate.push_back(new texcoord(u,1-v));       //I push back 1-v instead of normal v, because obj file use the upper left corner as 0,0 coorinate but OpenGL use bottom left corner as 0,0, so I convert it
                        istexture=true;
                }
        }
